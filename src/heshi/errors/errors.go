...
func GetMessage(err interface{}) string {
	...
	default:
		return fmt.Sprintf("Passed a non-error to GetMessage %v", err)
	}
}

// This returns a string with all available error information, including inner
// errors that are wrapped by this errors.
func (e *HeshiBaseError) Error() string {
	return DefaultError(e)
}

// This returns the error message without the stack trace.
func (e *HeshiBaseError) GetMessage() string {
	return e.Msg
}

// This returns the stack trace without the error message.
func (e *HeshiBaseError) GetStack() string {
	return e.Stack
}

// This returns the stack trace's context.
func (e *HeshiBaseError) GetContext() string {
	return e.Context
}

// This returns the wrapped error, if there is one.
func (e *HeshiBaseError) GetInner() error {
	return e.inner
}

// This returns a new HeshiBaseError initialized with the given message and
// the current stack trace.
func New(msg string) HeshiError {
	stack, context := StackTrace()
	return &HeshiBaseError{
		Msg:     msg,
		Stack:   stack,
		Context: context,
	}
...
func Newf(format string, args ...interface{}) HeshiError {
	// stack, context := StackTrace()
	// return &HeshiBaseError{
...
// Mark error in a new HeshiBaseError.
func Mark(err error) HeshiError {
	stack, context := StackTrace()
	return &HeshiBaseError{
		Stack:   stack,
		Context: context,
...
// Wraps another error in a new HeshiBaseError.
func Wrap(err error, msg string) HeshiError {
	stack, context := StackTrace()
	return &HeshiBaseError{
		Msg:     msg,
		Stack:   stack,
...
func Wrapf(err error, format string, args ...interface{}) HeshiError {
	// stack, context := StackTrace()
	// return &HeshiBaseError{
	// 	Msg:     fmt.Sprintf(format, args...),
	// 	Stack:   stack,
...
func DefaultError(e HeshiError) string {
