  ...
  <div v-for="(diamond,index) in diamonds" id="dia-data-box" :key="diamond.id">
    <div class="dia-piece-box">
      ...
      <div :class="index" class="generalinfobox">
        ...
        <span class="valuetxt value_color">颜色 <span class="thevalue">{{ diamondColor(diamond.color) }}</span>
        </span>
        ...
        <span class="detail-btn" v-on:click="'/openDiaDetail?id=' + diamond.id"><span class="glyphicon glyphicon-eye-open"></span> 详情</span>
      ...
      <div :id="'d_'+index" class="details" :class ="'details_'+diamond.id"><!-- detail box -->
        <div class="detailcol detailcol1"><!-- TODO handle fancycolor -->
          ...
          <img v-else class="dia-demo-pic" :src="'/_images/constant/'+detail_forDiamond_byShape(diamond.shape,'PICTURE')"/>
          <img v-if="diamond.grading_lab==='HRD'" class="certi-demo-pic" src="/_images/constant/hrd-label.jpg" />
          <img v-else-if="diamond.grading_lab==='GIA'" class="certi-demo-pic" src="/_images/constant/gia-label.jpg" />
          <img v-else-if="diamond.grading_lab==='IGI'" class="certi-demo-pic" src="/_images/constant/igi-label.jpg" />
        </div>
        <div class="detailcol detailcol2">
          ...
          <span>
            <a v-if="diamond.grading_lab==='HRD'" class="certi_linker" target="_blank" :href="'https://my.hrdantwerp.com/?L=&record_number=' + diamond.certificate_number+'&certificatetype=MC'">查看证书 <span class="glyphicon glyphicon-new-window"></span></a>
        ...
        <div class="detailcol detailcol3"><!-- TODO handle diamonds page with query in url -->
          ...
          <div v-if="diamond.profitable=='YES'">
            ...
            <div v-if="diamond.status==='SOLD'">
              <button type="button" class="btnfororder disabledbtn itemordered" title="chosenitem" disabled="disabled"><span class="glyphicon glyphicon-globe"></span> 该钻已售</button>
            </div>
            <div v-else>
              ...
              <div v-else-if="in_dia_items_shoppinglist">
                <p  class="actionbuttonsbox interested-btn-box" id="interested-btn-box_<?php echo $row['stock_ref']; ?>">
                  <button type="button" class="btnfororder disabledbtn itemordered" title="chosenitem" disabled="disabled">
        ...
        <br class="clear" />
        <button class="closeDetailBtn" v-on:click="closeDiaDetail(diamond.stock_ref)">
