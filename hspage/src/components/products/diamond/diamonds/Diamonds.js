...
function filter_shape(theshape) {
  ...
  $('.fileber_shape_outer>li.btn-active').each(function () {
    ...
    $shape += ' shape = "' + crr_obj.attr('title') + '" ';
    hash_shape += crr_obj.attr('title');
    shapeChoiceCounter++;
  });
  console.log($shape);
  update();
  ...
  if (thecolor != 'Fancy') {
    ...
    $('.filter_color_outer>li.btn-active').each(function () {
      ...
      if (crrcolor == 'N-Z') {
        ...
        $color += ' color = "' + crr_obj.attr('title') + '" ';
        hash_color += crr_obj.attr('title');
      }
      colorChoiceCounter++;
    });
  } else {
...
function filter_clarity(theclarity) {
  ...
  $('.filter_clarity_outer>li.btn-active').each(function () {
    ...
    $clarity += ' clarity = "' + crr_obj.attr('title') + '" ';
    hash_clarity += crr_obj.attr('title');
    clarityChoiceCounter++;
  });
  console.log($clarity);
  update();
...
function filter_cut(thegrade) {
  ...
  $('.filter_cut_outer>li.btn-active').each(function () {
    ...
    $cut += ' cut_grade = "' + crr_obj.attr('title') + '" ';
    hash_cut += crr_obj.attr('title');
    cutChoiceCounter++;
  });
  console.log($cut);
  update();
...
function filter_polish(thegrade) {
  ...
  $('.filter_polish_outer>li.btn-active').each(function () {
    ...
    $polish += ' polish = "' + crr_obj.attr('title') + '" ';
    hash_polish += crr_obj.attr('title');
    ChoiceCounter++;
  });
  console.log($polish);
  update();
...
function filter_sym(thegrade) {
  ...
  $('.filter_sym_outer>li.btn-active').each(function () {
    ...
    $sym += ' symmetry = "' + crr_obj.attr('title') + '" ';
    hash_sym += crr_obj.attr('title');
    ChoiceCounter++;
  });
  console.log($sym);
...
function filter_certi(thelab) {
  ...
  $('.filter_certi_outer>li.btn-active').each(function () {
    ...
    $certi += ' grading_lab = "' + crr_obj.attr('title') + '" ';
    hash_certi += crr_obj.attr('title');
    certiChoiceCounter++;
  });
  console.log($certi);
  update();
...
function filter_place() {
  ...
  if (chosenPlace == 'CHINA') {
    ...
    $place += ' country LIKE "Belg%" OR country LIKE "Antwerp%" OR country = "" OR country IS NULL ';
  } else if (chosenPlace == 'INDIA') {
    $place += ' country LIKE "IND%" ';
  } else if (chosenPlace == 'OTHER') {
    $place += ' country NOT LIKE "China" AND country NOT LIKE "SZ" AND country NOT LIKE "HK" AND country NOT LIKE "HSTHK" AND country NOT LIKE "Belg%" AND country NOT LIKE "IND%"  AND country <> ""  AND country IS NOT NULL ';
...
function filter_weight() {
  ...
  hash_weight_from = $weight_from;
  hash_weight_to = $weight_to;

  $("#slider-range-weight").slider("values", 0, $weight_from);
  $("#slider-range-weight").slider("values", 1, $weight_to);
...
function filter_price() {
  ...
  hash_price_from = $price_from;
  hash_price_to = $price_to;

  if ($price_from < 9800) {
    var crr_v_slider_l = Math.round((parseFloat($price_from) + 400) / 50);
  } else {
    var crr_v_slider_l = Math.round(Math.sqrt((parseFloat($price_from) - 9050) / 68) + 188);
  }
  $("#slider-range").slider("values", 0, crr_v_slider_l);


  if ($price_to < 9800) {
    var crr_v_slider_r = Math.round((parseFloat($price_to) + 400) / 50);
  } else {
    var crr_v_slider_r = Math.round(Math.sqrt((parseFloat($price_to) - 9050) / 68) + 188);
  }
...
function sortingby(ref) {
  ...
  if ($('li#sortingby_' + ref).hasClass('DESC')) {
    $sorting_direction = 'DESC';
    hash_sorting_direction = 'DESC';
  } else {
    $sorting_direction = 'ASC';
    hash_sorting_direction = 'ASC';
  }
...
function vatChoice() {
  if ($('input#vat-choice').prop("checked")) {
    $vat_include = 'YES';
    hash_vat_include = 'YES';
  } else {
    $vat_include = 'NO';
...
function update() {
  var hashSTRToSet = 'shape=' + hash_shape + '&color=' + hash_color + '&clarity=' + hash_clarity + '&cut=' + hash_cut + '&polish=' + hash_polish + '&sym=' + hash_sym + '&fluo=' + hash_fluo + '&certi=' + hash_certi + '&weight_from=' + hash_weight_from + '&weight_to=' + hash_weight_to + '&price_from=' + hash_price_from + '&price_to=' + hash_price_to + '&sorting=' + hash_sorting + '&sorting_direction=' + hash_sorting_direction + '&crr_page=' + hash_crr_page + '&vat_choice=' + hash_vat_include + '&place=' + hash_place;
  location.hash = hashSTRToSet;
  $('#loadingtxt').html('载入中...');
  $('div#loadingcover').fadeIn('fast');
  t = setTimeout('stopLoadingContent()', 7899);
  updatingstatus = 'updating';
  xhr = $.post(
    "/_content/ajax/diamondsdata.php",
    { shape: $shape, color: $color, clarity: $clarity, cut: $cut, polish: $polish, sym: $sym, fluo: $fluo, place: $place, certi: $certi, weight_from: $weight_from, weight_to: $weight_to, price_from: $price_from, price_to: $price_to, sorting: $sorting, sorting_direction: $sorting_direction, crr_page: $crr_page, vat_choice: $vat_include },
    function (data) {
...
function searchbyRef() {
  ...
  var postURL = '/_content/ajax/diamonddata-byref.php';
  ...
  var hashSTRToSet = 'ref=' + $.trim(theRef);
  location.hash = hashSTRToSet;


  $('div#loadingcover').fadeIn('fast');
  t = setTimeout('stopLoadingContent()', 3899);
  updatingstatus = 'updating';
  xhr = $.post(
    postURL,
    { ref: theRef, vat_choice: $vat_include },
    function (data) {
      updatingstatus = 'updated';
      clearTimeout(t);
      if (data == 'NO-DIAMOND-FOUND') {
...
function closeDiaDetail(theRef) {
...
function makeorder_confirmed(theRef) {
  var itemtype = "DIAMOND";
  $('div#loadingcover').fadeIn('fast');
  $.post(
    "/_includes/functions/addtoshoppinglist.php",
...
function mobileFunctions() {
  ...
  $(document).on("vmousemove", '#slider-range-weight span.weight_from_handel', function (event) {
    ...
    t_update = setTimeout(filter_weight, 288);
  });
  ...
  $(document).on("vmousemove", '#slider-range span.price_from_handel', function (event) {
    ...
    clearTimeout(t_update);
    t_update = setTimeout(filter_price, 288);
  });
...
function showOrderTips() {
  //alert($('div#thelettertoagents-container').length);
...
function showOrderSuccessFeedback() {
  $('div#feedbackcover').fadeIn(500);
  //crr_ordered_diamond
}

function gotoMountingsfor() {
  var mountingURL = '/addringfordiamond.php?ref=' + crr_ordered_diamond + '&ordered=yes';
  window.open(mountingURL, '_self');
}
function xtheSfdb() {
