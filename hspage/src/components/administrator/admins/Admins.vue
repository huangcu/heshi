<template>
  <div>
    ...
    <div v-if="activeAdminUsers===''">
      <p style="text-align:center;">没有找到用户</p>
    </div>
    <div v-else>
      <ul v-if="activeAdminUsers.length!==0" id="userlist" class="userlist">
        <li v-for="(user, index) in activeAdminUsers" :key="index" class="user-record">
          ...
          <a class="editbtn" href="editadmin">修改</a>
          <button class="deletebtn" v-on:click="deleteAdmin(user.id)">删除</button>
        </li>
      </ul>
      <p v-if="adminprofile.admin.admin_level===1" style="text-align:center;">
        <a id="new-btn" href="newadmin">
